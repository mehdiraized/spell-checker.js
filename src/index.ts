...
class SpellChecker {
  ...
  private dictionaries = {
    ...
    fa: {
      src: resolve(__dirname, "../dictionaries/fa/farsi.txt"),
      charset: "utf8"
    }
  };
  ...
  public load(inputOrProps, charsetOption?) {
    ...
    if (!options.async) {
      const dictionary: FileResult = this.readDictionarySync(options.input, options.charset, this.BUFFER.WORDS);
   ...
   * @return {Object}
   */
  private readDictionarySync(input: string, charset: string, words: Set<string>): FileResult {
   ...
   * @return {Object}
   */
  private async readDictionaryAsync(input: string, charset: string, words: Set<string>): Promise<FileResult> {
    if (this.dictionaries[input] != null) {
      charset = this.dictionaries[input].charset;
      input = this.dictionaries[input].src;
    }
    ...
    const wordsList: FileResult = this.getWordsList(fileBuff, charset, words);
